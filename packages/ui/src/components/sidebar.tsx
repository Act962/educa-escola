...
import { Tooltip, TooltipContent, TooltipTrigger } from "@educa-escola/ui/components/tooltip";
import { useIsMobile } from "@educa-escola/ui/hooks/use-mobile";
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // renderizar a sidebar já no estado certo. Com localStorage a barra
      // piscaria aberta antes de recolher. `cookieStore` ainda não é
      // universal, então fica o `document.cookie`.
      // biome-ignore lint/suspicious/noDocumentCookie: precisa ser legível no SSR
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      <div
        ...
        className={cn(
          "group/sidebar-wrapper mx-auto flex min-h-svh w-full max-w-[1560px] bg-background",
          className,
        )}
        {...props}
      >
...
/**
 * Sidebar do shadcn, ajustada à identidade do Integra Edu.
 *
 * Usada na variante `floating`: casca branca de 24px de raio sobre o azul do
 * app, como no mockup — sem sombra nem anel, que é a regra do UI kit.
 *
 * É ela que resolve a tela estreita: a barra vira um `Sheet` acionado pelo
 * `SidebarTrigger`, em vez de uma barra inferior de app mobile. O produto é
 * web, e a navegação continua sendo a lateral.
 */
function Sidebar({
  side = "left",
  variant = "sidebar",
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          variant === "floating" || variant === "inset"
            ? "group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(10)))]"
      ...
      <div
        ...
        className={cn(
          ...
          variant === "floating" || variant === "inset"
            ? "p-4 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(10))+2px)] md:p-5"
      ...
      >
        <div
          ...
          className="flex size-full flex-col gap-4 overflow-y-auto bg-sidebar p-4 group-data-[variant=floating]:rounded-shell group-data-[collapsible=icon]:p-2"
        >
          {children}
        </div>
      </div>
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex w-full min-w-0 flex-1 flex-col gap-5 bg-background p-4 md:py-5 md:pr-5 md:pl-0",
        className,
      )}
      {...props}
    />
  ...
  "peer/menu-button group/menu-button flex w-full items-center gap-3 overflow-hidden rounded-control px-3 text-left font-semibold text-[13px] outline-hidden transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:outline-2 focus-visible:outline-ring focus-visible:outline-offset-2 active:bg-sidebar-accent disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:cursor-not-allowed aria-disabled:opacity-60 data-active:bg-sidebar-primary data-active:font-bold data-active:text-sidebar-primary-foreground data-open:hover:bg-sidebar-accent group-data-[collapsible=icon]:size-11! group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:px-0! [&>span:last-child]:truncate group-data-[collapsible=icon]:[&>span:last-child]:hidden [&_svg]:size-[18px] [&_svg]:shrink-0",
  {
    variants: {
      ...
      size: {
        default: "min-h-11",
        sm: "min-h-9 text-xs",
        lg: "min-h-14 group-data-[collapsible=icon]:p-0!",
      },
    },
    defaultVariants: {
      variant: "default",
