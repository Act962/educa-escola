...
/**
 * Tabs do shadcn, ajustadas ao trilho segmentado do Integra: raio de 14px no
 * trilho, 11px no item, e o item ativo em azul cheio, como no mockup.
 */
...
const tabsListVariants = cva(
  "group/tabs-list inline-flex w-fit items-center justify-center rounded-control p-1 text-muted-foreground data-[variant=line]:rounded-none group-data-vertical/tabs:h-fit group-data-vertical/tabs:flex-col",
  {
    variants: {
      variant: {
        default: "bg-muted",
...
function TabsContent({ className, ...props }: TabsPrimitive.Panel.Props) {
  return (
    <TabsPrimitive.Panel
      ...
      className={cn("flex-1 text-corpo outline-none", className)}
      {...props}
    />
  );
}
