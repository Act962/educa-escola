...
describe("gerarCodigo", () => {
  it("começa com duas letras do nome, para a família se reconhecer", () => {
    expect(generateCode("Maria Clara", new Set(), sempreZero)).toMatch(/^MA/);
  });
  ...
  it("nunca usa símbolo ambíguo", () => {
    ...
    for (let i = 0; i < 300; i += 1) {
      expect(generateCode("Ana Sofia", new Set())).not.toMatch(proibidos);
    }
  });
  ...
  it("gera código mesmo sem prefixo possível", () => {
    expect(generateCode("123", new Set(), sempreZero)).toHaveLength(6);
  });
...
describe("normalizarCodigo", () => {
  ...
  it("aceita o que a pessoa digitou de verdade", () => {
    expect(normalizeCode("  ma-4k2z ")).toBe("MA4K2Z");
    expect(normalizeCode("MA 4K 2Z")).toBe("MA4K2Z");
    expect(normalizeCode("ma4k2z")).toBe("MA4K2Z");
  });

  it("não deixa entrada gigante virar consulta gigante", () => {
    expect(normalizeCode("A".repeat(500))).toHaveLength(24);
  });
});
